<template>
  <div class="template">
    ...
    <div class="templateContent" ref="templateContent">
      ...
      <div class="templateContentDiv" v-for="(item,index) in historyData">
        <div class="templateContentDivLeft">
          <span>{{item.text}}</span>
      ...
      <div class="templateContentDiv" v-for="(item,index) in payData">
        <div class="templateContentDivLeft">
          <span>{{item.text}}</span>
          <span>{{item.measurement}}</span>
        </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        payTime:"",
        payData:[
          {"text":"总数","measurement":"(万千万时)","value":""},
        ]
    ...
    methods: {
      ...
      //保存数据
      doSaveFillInDay(){
        if(this.historyTime && this.payTime){
          axios.post(" " + URL + "/app/Electricity/fillInMonthData", {
